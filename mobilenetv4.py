...
Code from: https://github.com/d-li14/mobilenetv4.pytorch
"""
...
def mobilenetv4_conv_small_grayscale(num_classes=10):
    """
    Modified MobileNetV4-Conv-Small for grayscale input (1 channel) and 10 output classes.
    """
    block_specs = [
        # conv_bn, kernel_size, stride, out_channels
        # uib, start_dw_kernel_size, middle_dw_kernel_size, stride, out_channels, expand_ratio
        # 112px
        ('conv_bn', 3, 2, 32),  # First layer now accepts 1 input channel
        # 56px
        ('conv_bn', 3, 2, 32),
        ('conv_bn', 1, 1, 32),
        # 28px
    ...
    # Create model with 3 input channels (original RGB)
    model = MobileNetV4(block_specs, num_classes=num_classes)

    # Replace the first Conv2d layer to accept 1 input channel
    original_conv = model.features[0].block[0]  # Original Conv2d layer (3 input channels)
    new_conv = nn.Conv2d(
        in_channels=1,
    ...
    # Copy weights: Average 3 input channels to 1 (grayscale)
    new_conv.weight.data = original_conv.weight.data.mean(dim=1, keepdim=True)

    # Replace the original Conv2d layer
    model.features[0].block[0] = new_conv
